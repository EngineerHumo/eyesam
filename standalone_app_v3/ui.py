...
class MainWindow:
    def __init__(self, root: tk.Tk, pipeline: SurgicalPipeline):
        ...
        self.spot_distance = DEFAULT_SPOT_DISTANCE
        self.exposure_time = 0
        self.spot_diameter_var = tk.IntVar(value=self.spot_diameter)
        self.spot_distance_var = tk.IntVar(value=self.spot_distance)
    ...
    def _setup_ui(self) -> None:
        ...
        self.scheme_frame = scheme_frame
    ...
    def _setup_sliders(self) -> None:
        ...
        self._add_slider(
            self.slider_frame,
            "光斑直径",
            self.spot_diameter_var,
            MIN_SPOT_DIAMETER,
            MAX_SPOT_DIAMETER,
            self._on_spot_setting_change,
        )
        self._add_slider(
            self.slider_frame,
            "光斑距离",
            self.spot_distance_var,
            MIN_SPOT_DISTANCE,
            MAX_SPOT_DISTANCE,
            self._on_spot_setting_change,
        )
    ...
    def _on_spot_setting_change(self, _value: str) -> None:
        self.spot_diameter = int(self.spot_diameter_var.get())
        self.spot_distance = int(self.spot_distance_var.get())
    ...
    def open_image(self) -> None:
        ...
        self.spot_diameter = DEFAULT_SPOT_DIAMETER
        self.spot_distance = DEFAULT_SPOT_DISTANCE
        self.state.current_mask = scheme_masks[0] if scheme_masks else None
        self.state.current_logits = logits
        self.state.auto_click = last_auto_click
        self.state.clicks = [last_auto_click] if last_auto_click else []
    ...
    def _rebuild_scheme_plans(self) -> None:
        if self.original_pil is None or not self.schemes:
            return
        if self.faz_center is None:
            self.faz_center = (self.original_pil.width // 2, self.original_pil.height // 2)
    ...
    def _assign_remaining_circles(self) -> None:
        if self.original_pil is None or self.area_mask is None or not self.schemes:
            return
        if self.faz_center is None:
            self.faz_center = (self.original_pil.width // 2, self.original_pil.height // 2)
        area_bin = binarize_mask(self.area_mask)
        area_plan = plan_surgery(
            self.original_pil,
            area_bin,
            self.faz_center,
            area_mask=self.area_mask,
        ...
        all_centers = set(area_plan.circle_centers)
        assigned_centers = set()
        for scheme in self.schemes:
            assigned_centers.update(scheme.circles)
        unassigned = list(all_centers - assigned_centers)
        if not unassigned:
            return

        distance_maps = []
        for scheme in self.schemes:
            mask_bin = binarize_mask(scheme.mask)
            if int(np.sum(mask_bin)) == 0:
                distance_maps.append(None)
                continue
            inverse = (1 - mask_bin).astype(np.uint8)
            distance_maps.append(cv2.distanceTransform(inverse, cv2.DIST_L2, 5))

        for center in unassigned:
            x, y = center
            best_idx = 0
            best_dist = None
            for idx, dist in enumerate(distance_maps):
                if dist is None:
                    continue
                ...
                if best_dist is None or value < best_dist:
                    best_dist = value
                    best_idx = idx
            self.schemes[best_idx].circles.append(center)

    def _scheme_label(self, index: int) -> str:
        chinese = [
            "一",
            "二",
            "三",
    ...
    def _refresh_scheme_buttons(self) -> None:
        for widget in self.scheme_button_container.winfo_children():
            widget.destroy()
        self.scheme_buttons = []
        for idx, _scheme in enumerate(self.schemes):
            btn = tk.Button(
                self.scheme_button_container,
                text=self._scheme_label(idx),
                width=8,
                command=lambda i=idx: self._toggle_scheme_selection(i),
            )
            btn.pack(pady=4)
    ...
    def _render_scheme_overlay(self) -> None:
        ...
        circle_radius = self._circle_radius()
        center_map: dict[Tuple[int, int], List[int]] = {}
        for idx, scheme in enumerate(self.schemes):
            for center in scheme.circles:
    ...
    def on_canvas_click(self, event) -> None:
        ...
        if self.faz_center is None:
            self.faz_center = (self.original_pil.width // 2, self.original_pil.height // 2)
        display_mask = resize_mask(result.mask, (self.original_pil.width, self.original_pil.height))
        display_mask = self._apply_area_constraint(display_mask)
    ...
    def _apply_plan(self, result, plan: PlanResult, display_mask: np.ndarray) -> None:
        ...
        if self.schemes:
            self.schemes[0].mask = display_mask
            self._rebuild_scheme_plans()
        else:
            self.plan = plan
            self._render_overlay(plan.overlay)
    ...
    def _apply_point_modification(self, point: Tuple[int, int], add: bool) -> None:
        ...
        self.state.current_mask = self._apply_area_constraint(self.state.current_mask)
        if self.faz_center is None:
            self.faz_center = (self.original_pil.width // 2, self.original_pil.height // 2)
        plan = plan_surgery(
            self.original_pil,
        ...
        if self.schemes:
            self.schemes[0].mask = self.state.current_mask
            self._rebuild_scheme_plans()
        else:
            self.plan = plan
            self._render_overlay(plan.overlay)
    ...
    def _postprocess_first_mask(self, mask: np.ndarray) -> np.ndarray:
        ...
        if self.area_mask is not None:
            area_bin = binarize_mask(self.area_mask)
            filled = (filled > 0).astype(np.uint8) * area_bin
        return filled

    def _apply_area_constraint(self, mask: np.ndarray) -> np.ndarray:
