...
import (
	...
	"eyesam/standalone_app_v3_6_go/internal/planner"
	"eyesam/standalone_app_v3_6_go/internal/preprocess"
...
func New(onnxDir string) *Pipeline {
	...
	if !fileExists(tmpPre) || !fileExists(tmpFirst) || !fileExists(tmpIter) {
		log.Fatal("模型文件不存在", tmpPre, tmpFirst, tmpIter)
	}
...
func (p *Pipeline) RunInitial(img image.Image, progress func(int)) (Result, error) {
	...
	log.Printf("yong auto_click0=(%d,%d)", click0.X, click0.Y)
	...
	for idx := 0; idx < 4; idx++ {
		...
		log.Printf("yong1 auto_click%d=(%d,%d)", idx+1, currentClick.X, currentClick.Y)
