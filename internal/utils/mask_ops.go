...
// SaveMaskAsPNG 将 Mask 结构体保存为 PNG 文件
func SaveMaskAsPNG(m Mask, baseName string) error {
	// 1. 准备文件名和图像对象
	fileCounter++
	fileName := fmt.Sprintf("%s_%d.png", baseName, fileCounter)
	img := image.NewGray(image.Rect(0, 0, m.Width, m.Height))

	// 2. 长度检查：确保数据完整
	if len(m.Data) != len(img.Pix) {
		return fmt.Errorf("数据长度不匹配: Data(%d) vs Pix(%d)", len(m.Data), len(img.Pix))
	}

	// 3. 填充像素并进行颜色映射 (0/1 -> 0/255)
	for i := 0; i < len(m.Data); i++ {
		if m.Data[i] == 1 {
			img.Pix[i] = 255 // 转换为白色可见
		} else {
			img.Pix[i] = 0 // 保持黑色
		}
	}
...
func InscribedCenter(mask Mask) image.Point {
	...
	fmt.Printf("maxValue: %d, idx: %d point(%d, %d)\n", maxValue, idx, idx/mask.Width, idx%mask.Width)
