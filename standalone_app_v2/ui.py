...
class MainWindow:
    ...
    def on_canvas_click(self, event) -> None:
        ...
        if not self.state.has_plan and click.label == 1:
            ...
            plan = plan_surgery(
                self.original_pil, display_mask, self.faz_center, area_mask=self.area_mask
            )
        ...
        plan = plan_surgery(self.original_pil, display_mask, self.faz_center, area_mask=self.area_mask)
        self._apply_plan(result, plan, display_mask)
        self._start_preview_loop()
    ...
    def _apply_point_modification(self, point: Tuple[int, int], add: bool) -> None:
        ...
        if self.faz_center is None:
            self.faz_center = (self.original_pil.width // 2, self.original_pil.height // 2)
        plan = plan_surgery(
            self.original_pil, self.state.current_mask, self.faz_center, area_mask=self.area_mask
        )
        self.plan = plan
        self._render_overlay(plan.overlay)
