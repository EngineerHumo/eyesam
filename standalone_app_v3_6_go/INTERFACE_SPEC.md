...
**示例**
```go
planner := eyesam.NewPlanner("standalone_app_v3_6_go/onnx")
result, err := planner.InitialPlan("demo.png", "output/run1")
```
...
**示例**
```go
err := planner.SelectInitialScheme("output/run1", 0)
```
...
**示例**
```go
maskPath, logitsPath, clicksPath, err := planner.IterateWithClicks("output/run1", "new_clicks.json")
```
...
**示例**
```go
logitsPath, err := planner.AddClickPoint("output/run1", "add_point.json")
```
...
**示例**
```go
logitsPath, err := planner.RemoveClickPoint("output/run1", "remove_point.json")
```
...
**示例**
```go
logitsPath, err := planner.AddArea("output/run1", "add_area.json")
```
...
**示例**
```go
logitsPath, err := planner.RemoveArea("output/run1", "remove_area.json")
```
...
**示例**
```go
err := planner.ClearCurrent("output/run1")
```
