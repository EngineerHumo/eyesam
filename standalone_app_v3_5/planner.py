...
def plan_surgery(
    ...
    spot_diameter: int = DEFAULT_SPOT_DIAMETER,
    spot_distance: int = DEFAULT_SPOT_DISTANCE,
) -> PlanResult:
    overlay = image.copy()
    ...
    h, w = mask_bin.shape
    cx, cy = faz_center
    radius_step = int(round(spot_diameter + spot_distance))
    min_distance = max(1, int(round(spot_diameter + spot_distance)))
    circle_radius = max(1, int(round(spot_diameter / 2)))
    max_radius = int(
        max(
            math.hypot(cx, cy),
    ...
    while radius <= max_radius:
        ring_points = generate_ring_points(faz_center, radius, min_distance=min_distance)
