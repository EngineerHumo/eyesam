...
import (
	...
	"rakrock/app/business/rakai/prp/internal/npy"
	"rakrock/app/business/rakai/prp/internal/pipeline"
...
var pipelineIns *pipeline.Pipeline = nil
...
func NewSurgicalInterface(onnxDir string) (*SurgicalInterface, error) {
	...
	if pipelineIns == nil {
		pipelineIns = pipeline.New(onnxDir)
		err := pipelineIns.FirstModel.Init()
		if err != nil {
			logger.Error("Failed to init FirstModel onnx", "err", err)
			return nil, err
		}
		err = pipelineIns.IterationModel.Init()
		if err != nil {
			logger.Error("Failed to init IterationModel onnx", "err", err)
			return nil, err
		}
		err = pipelineIns.PreModel.Init()
		if err != nil {
			logger.Error("Failed to init IterationModel onnx", "err", err)
			return nil, err
		}
	}
	return &SurgicalInterface{pipeline: pipelineIns}, nil
}
...
func (s *SurgicalInterface) loadClicks(path string) ([]Click, error) {
	if !fileExists(path) {
		logger.Error("clicke file is miss", "path", path)
...
func (s *SurgicalInterface) ApplyClicks(clicksJSON string) (map[string]string, error) {
	...
	if err != nil {
		...
		logger.Info("ApplyClicks", "input clicks", newClicks)
	}
	...
	logger.Info("Apply Clicks", "clicks", clicks)
