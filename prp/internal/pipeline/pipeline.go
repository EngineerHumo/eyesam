...
import (
	...
	"rakrock/app/business/rakai/prp/internal/inference"
	"rakrock/app/business/rakai/prp/internal/planner"
	"rakrock/app/business/rakai/prp/internal/preprocess"
	"rakrock/app/business/rakai/prp/internal/utils"
)

var logger = log.Logger.Named("pipeline")
...
func New(onnxDir string) *Pipeline {
	tmpPre := filepath.Join(onnxDir, "pre.onnx")
	tmpFirst := filepath.Join(onnxDir, "first.onnx")
	tmpIter := filepath.Join(onnxDir, "iteration.onnx")

	if !fileExists(tmpPre) || !fileExists(tmpFirst) || !fileExists(tmpIter) {
		logger.Error("模型文件不存在", tmpPre, tmpFirst, tmpIter)
		return nil
	}

	return &Pipeline{
		PreModel:       preprocess.NewPreSegmenter(tmpPre),
		FirstModel:     inference.NewOnnxModel(tmpFirst),
		IterationModel: inference.NewOnnxModel(tmpIter),
	}
}
...
func (p *Pipeline) RunInitial(img image.Image, progress func(int)) (Result, error) {
	...
	for idx := 0; idx < 4; idx++ {
		...
		logger.Info("auto_click:", "idx", idx+1, "x", currentClick.X, "y", currentClick.Y)
...
func (p *Pipeline) runFirstWithClick(firstImage utils.ModelImage, resizedHW [2]int, click image.Point) (inference.Result, utils.Click, error) {
	...
	logger.Info("auto_click: ", "x", click.X, "y", click.Y)
...
func (p *Pipeline) findMaskInputShape(resizedHW [2]int) [2]int {
	for name, shape := range p.IterationModel.InputShapes {
		if strings.Contains(name, "mask_inputs") && len(shape) == 4 && shape[1] == 1 {
			h := int(shape[2])
			w := int(shape[3])
			if h > 0 && w > 0 {
				return [2]int{h, w}
